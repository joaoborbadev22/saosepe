...
const MapFlat = ({navigation}) => {
    ...
    getMaps = () => {
        fetch('http://env-9593657.jelastic.saveincloud.net/Perso')
            ...
            renderItem={({item})=> 
              (
                <View style={{
                  ...
                  >
                  <Pressable onPress={()=> modalfr(item.id)} style={{height:'100%',justifyContent:'center',alignItems:'center',position:'relative'}}><Image style={{width:"100%",height:"100%",borderRadius: 12,}} source={require('../../src/jornal.jpg')}/></Pressable>
                </View>
              ) 
            }
          />
        ...
        <Modal animationType="slide" visible={showModal} onRequestClose={() => setShowModal(false)}>
          <ScrollView>
              <View style={styles.header}>
                  <Image style={{width:"100%",height:"100%"}} source={require('../../src/jornal.jpg')}/>
              </View>
              <View style={{width:'100%',padding:15,justifyContent:'center',minHeight:90,alignItems:'center'}}>
                  {maps.map(a=>{
                      return option === a.id && <Text style={{fontSize:18,fontFamily:'Inter_600SemiBold',textAlign:'center'}}>{a.title}</Text>
                  })}
